...
const SECOES_CATALOGO_PUBLICO = new Set([5, 14])
const MARCA_RAMASSOL = 'ramassol'
...
function ehProdutoRamassol(produto) {
  return normalizarTexto(produto?.marca).includes(MARCA_RAMASSOL)
}

function produtoCasaBuscaGeral(produto, buscaNorm) {
  ...
  return (
    nome.includes(buscaNorm) ||
    descricao.includes(buscaNorm) ||
    marca.includes(buscaNorm) ||
    codigo.includes(buscaNorm)
  )
}
...
export function imagemUrlProduto(produto) {
  const versao = `v=${new Date().toISOString().slice(0, 13)}`
  if (produto?.__fallback && produto?.fallback_foto_url) return produto.fallback_foto_url
  const nome = normalizarTexto(produto?.nome)
  const regra = FOTO_PREFIXO_MAP.find((item) => nome.startsWith(item.prefixo))
  if (regra) return `${API_URL}/fotos/${regra.produtoIdFoto}.jpg?${versao}`
...
async function fetchFallbackJson(file, { noStore = false, revalidate = PUBLIC_CACHE_SECONDS } = {}) {
  try {
...
async function getFallbackCatalogSnapshot({ noStore = false, revalidate = PUBLIC_CACHE_SECONDS } = {}) {
...
export async function getCatalogoCompletoComFallback({
  noStore = false,
  revalidate = PUBLIC_CACHE_SECONDS,
} = {}) {
  const apiBase = await fetchJson(
    `/produtos?skip=0&limit=10000&todas_secoes=1`,
    noStore ? { cache: 'no-store' } : { next: { revalidate } },
    null
  )
  const fallbackBase = await getFallbackCatalogSnapshot({ noStore, revalidate })
  const mapa = new Map()

  for (const item of [...(fallbackBase || []), ...((apiBase?.produtos || []))]) {
...
function filtrarProdutosLocalmente(
  ...
  if (!todas_secoes && secao == null) {
    lista = lista.filter(
      (item) => SECOES_CATALOGO_PUBLICO.has(Number(item?.secao || 0)) || ehProdutoRamassol(item)
    )
  }

  if (secao != null && secao !== '') {
    lista = lista.filter(
      (item) => Number(item?.secao || 0) === Number(secao) || ehProdutoRamassol(item)
    )
  }
  ...
  if (busca) {
    ...
    lista = lista.filter((item) => {
      if (ehProdutoRamassol(item)) return true
      return produtoCasaBuscaGeral(item, buscaNorm)
    })
  }
  ...
  if (marca) {
    ...
    lista = lista.filter(
      (item) => normalizarTexto(item?.marca).includes(marcaNorm) || ehProdutoRamassol(item)
    )
  }

  if (grupo != null && grupo !== '') {
    lista = lista.filter((item) => Number(item?.grupo || 0) === Number(grupo) || ehProdutoRamassol(item))
  }

  if (subgrupo != null && subgrupo !== '') {
    lista = lista.filter(
      (item) => Number(item?.subgrupo || 0) === Number(subgrupo) || ehProdutoRamassol(item)
    )
  } else if (subgrupos?.length) {
    const conjunto = new Set(subgrupos.map((item) => Number(item)))
    lista = lista.filter(
      (item) => conjunto.has(Number(item?.subgrupo || 0)) || ehProdutoRamassol(item)
    )
  }

  if (com_preco) {
    lista = lista.filter((item) => Number(item?.preco || 0) > 0 || ehProdutoRamassol(item))
  }

  if (em_estoque === true) {
    lista = lista.filter((item) => Number(item?.estoque || 0) > 0 || ehProdutoRamassol(item))
...
export async function getProdutos({
  ...
  const fallbackProdutos = await getFallbackCatalogSnapshot({ noStore, revalidate })
...
export async function getProdutosDestaque({ limit = 8, meses = 3, preco_min = 100 } = {}) {
  ...
  const fallbackProdutos = await getFallbackCatalogSnapshot({ revalidate: PUBLIC_CACHE_SECONDS })
...
export async function getProdutosCatalogoPorSubgrupo(subgrupo, { limit = 24, em_estoque = true, com_preco = false } = {}) {
  ...
  const apiResult = await fetchJson(
    `/produtos/subgrupo/${subgrupo}/catalogo?${qs}`,
...
export async function getHomeConfig() {
  ...
  const fallback = await fetchFallbackJson('home-config.json', {
    revalidate: PUBLIC_CACHE_SECONDS,
  })
  return (
    fallback?.data || {
      ...
      hero_images: [],
      logo_url: null,
      hero_title: 'Ofertas em aço para sua obra',
      hero_subtitle: 'Estoque real, preço atualizado e atendimento rápido no WhatsApp.',
      sections: {},
    }
  )
}
...
export async function vendedorListarOrcamentos(login, busca = '') {
  const qs = new URLSearchParams()
  if (login) qs.set('login', login)
  if (busca) qs.set('busca', busca)
  qs.set('limit', '300')
  return await fetchJson(`/vendedor/orcamentos?${qs}`, { cache: 'no-store' }, null)
}

export async function vendedorSalvarOrcamento(payload) {
  return await fetchJson(
    '/vendedor/orcamentos',
    {
      method: 'POST',
...
export async function vendedorListarOrcamentosDb2(busca, limit = 30) {
  const qs = new URLSearchParams()
  if (busca) qs.set('busca', busca)
  qs.set('limit', String(limit))
  return await fetchJson(`/vendedor/orcamentos-db2?${qs}`, { cache: 'no-store' }, null)
}

export async function vendedorObterOrcamentoDb2(empresa, numero) {
  return await fetchJson(
    `/vendedor/orcamentos-db2/${encodeURIComponent(empresa)}/${encodeURIComponent(numero)}`,
    { cache: 'no-store' },
    null
  )
}
