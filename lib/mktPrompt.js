import path from 'path'
import { readFile } from 'fs/promises'

const DEFAULT_MODEL = process.env.OPENAI_MKT_MODEL || 'gpt-image-1'
const SPEC_PATH = path.join(process.cwd(), 'docs', 'mkt-ad-guidelines.md')

function parseValorMonetario(valor) {
  if (typeof valor === 'number' && Number.isFinite(valor)) {
    return valor
  }

  const texto = String(valor ?? '').trim()
  if (!texto) return null

  const limpo = texto.replace(/[^\d,.-]/g, '')
  const separadores = [...limpo.matchAll(/[.,]/g)].map((match) => match.index)
  let normalizado = limpo

  if (separadores.length) {
    const ultimoSeparador = separadores[separadores.length - 1]
    const casas = limpo.length - ultimoSeparador - 1

    if (casas >= 1 && casas <= 2) {
      const decimalChar = limpo[ultimoSeparador]
      const parteInteira = limpo.slice(0, ultimoSeparador).replace(/[.,]/g, '')
      const parteDecimal = limpo.slice(ultimoSeparador + 1).replace(/[.,]/g, '')
      normalizado = `${parteInteira}.${parteDecimal}`
    } else {
      normalizado = limpo.replace(/[.,]/g, '')
    }
  }

  const numero = Number(normalizado)
  return Number.isFinite(numero) ? numero : null
}

function formatarValor(valor) {
  const numero = parseValorMonetario(valor)
  if (!Number.isFinite(numero) || numero <= 0) {
    return null
  }

  return new Intl.NumberFormat('pt-BR', {
    style: 'currency',
    currency: 'BRL',
  }).format(numero)
}

export async function carregarDiretrizMkt() {
  return readFile(SPEC_PATH, 'utf8')
}

function descreverFormato(postFormat) {
  if (postFormat === 'both') {
    return 'arte base versatil que funcione bem tanto para stories/status quanto para feed vertical 4:5'
  }
  return postFormat === 'feed'
    ? 'post para feed em formato vertical 4:5, elegante e compacto'
    : 'post para stories/status em formato vertical alto, com impacto imediato'
}

export async function montarPromptMkt({
  precoFormatado,
  nomeArquivo,
  nomeProduto,
  codigoProduto,
  postFormat = 'stories',
  withPrice = true,
}) {
  const diretriz = await carregarDiretrizMkt()

  return [
    'Voce e diretor de arte senior de varejo visual para a Galpao do Aco.',
    '',
    withPrice && precoFormatado ? `Preco oficial do anuncio: ${precoFormatado}.` : 'Este anuncio nao exibira preco final na arte.',
    nomeArquivo ? `Arquivo enviado pelo usuario: ${nomeArquivo}.` : null,
    nomeProduto ? `Nome comercial do produto: ${nomeProduto}.` : null,
    codigoProduto ? `Codigo interno do produto: ${codigoProduto}.` : null,
    `Formato final desejado: ${descreverFormato(postFormat)}.`,
    '',
    'Crie uma arte promocional vertical premium baseada fielmente no produto enviado.',
    'A arte deve parecer um anuncio profissional de loja de material de construcao, com composicao forte, contraste alto e foco comercial.',
    'Preserve a aparencia do produto, sem inventar acessorios, sem trocar cor principal e sem duplicar o item.',
    'Use um fundo contextual coerente com o uso do produto e deixe o produto dominante, heroico e central.',
    withPrice
      ? 'Deixe areas livres para o sistema aplicar a logo real e a faixa de preco depois:'
      : 'Deixe areas livres para o sistema aplicar a logo real e o rodape comercial depois:',
    '- reserve uma zona limpa no canto superior direito para a logo',
    withPrice
      ? '- reserve uma faixa inferior visualmente limpa, sem caixa vazia, sem tarja pronta e sem placeholder de preco'
      : '- reserve uma faixa inferior visualmente limpa, sem caixa vazia, sem tarja pronta e sem placeholder de preco',
    '',
    'Pode criar headline curta em portugues, em caixa alta, comercial e impactante.',
    withPrice ? 'Nao escreva o preco dentro da arte.' : 'Nao escreva preco dentro da arte.',
    'Nao desenhe logo parecida com a da marca.',
    'Nao desenhe caixa vermelha, quadro de preco, tarja promocional, retangulo vazio ou selo de valor.',
    'Nao coloque marcas de agua.',
    'Nao crie colagem baguncada.',
    'Nao diminua demais o produto; ele precisa parecer dominante e vender sozinho.',
    '',
    'Siga rigorosamente estas diretrizes de criacao:',
    diretriz,
  ]
    .filter(Boolean)
    .join('\n')
}

export function normalizarPrecoMkt(valor) {
  const precoFormatado = formatarValor(valor)
  if (!precoFormatado) {
    throw new Error('Informe um valor valido para o anuncio.')
  }

  return {
    precoFormatado,
    model: DEFAULT_MODEL,
  }
}

export function aplicarDescontoMkt(valor, descontoPercentual = 0) {
  const numeroBase = parseValorMonetario(valor)
  if (!Number.isFinite(numeroBase) || numeroBase <= 0) {
    throw new Error('Nao foi possivel calcular o preco do anuncio.')
  }

  const desconto = Number(descontoPercentual || 0)
  const fator = Math.max(0, 1 - desconto / 100)
  return Number((numeroBase * fator).toFixed(2))
}

export function sanitizarTextoCurto(valor, fallback = '') {
  return String(valor || fallback)
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 90)
}

export function montarPromptCopyMkt({ nomeProduto, precoFormatado, codigoProduto, postFormat = 'stories' }) {
  if (postFormat === 'both') {
    postFormat = 'feed'
  }

  if (postFormat === 'feed') {
    return [
      'Voce escreve legendas promocionais para feed do Instagram da Galpao do Aco.',
      `Produto: ${nomeProduto || 'Produto sem nome informado'}.`,
      codigoProduto ? `Codigo: ${codigoProduto}.` : null,
      `Preco final da oferta: ${precoFormatado}.`,
      '',
      'Escreva uma copy no padrao de legenda comercial pronta para postar no feed.',
      'A estrutura deve seguir exatamente este estilo:',
      '1. abertura falando com quem trabalha com o produto ou precisa da solucao',
      '2. nome do produto em destaque',
      '3. 3 linhas curtas com beneficios tecnicos/comerciais usando o simbolo ✔️',
      '4. preco em destaque com 💰 e o valor informado',
      '5. senso de urgencia com 🚨',
      '6. CTA para WhatsApp com 📲 e os telefones (95) 99165-0808 e (95) 3224-0115',
      '7. fechamento com 📍 Galpao do Aco e uma frase comercial forte',
      '',
      'Regras:',
      '- escrever em portugues natural e vendedor',
      '- pode usar emojis, mas so nesses blocos principais',
      '- nao usar hashtags',
      '- nao inventar caracteristicas tecnicas que nao estejam claras pelo nome do produto',
      '- manter o texto pronto para copiar e colar como legenda',
      '- retornar somente a copy final',
    ]
      .filter(Boolean)
      .join('\n')
  }

  return [
    'Voce escreve copys curtas para status e stories da Galpao do Aco.',
    `Produto: ${nomeProduto || 'Produto sem nome informado'}.`,
    codigoProduto ? `Codigo: ${codigoProduto}.` : null,
    `Preco final da oferta: ${precoFormatado}.`,
    `Formato da publicacao: ${postFormat === 'feed' ? 'feed' : 'stories/status'}.`,
    '',
    'Escreva uma copy promocional curta em portugues com exatamente 3 linhas.',
    'Cada linha deve ser curta, forte e comercial.',
    'A copy deve combinar com anuncio de loja de material de construcao.',
    'Nao use emojis.',
    'Nao use aspas.',
    'Nao use hashtags.',
    'Nao enumere.',
    'A ultima linha deve ter chamada para acao.',
    'Retorne apenas as 3 linhas da copy.',
  ]
    .filter(Boolean)
    .join('\n')
}
