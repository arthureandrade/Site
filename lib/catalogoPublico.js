...
export function inferirCategoriaProduto(produto) {
  const base = prepararTextoClassificacao(produto)

  for (const regra of REGRAS_PRIORITARIAS_CATEGORIA) {
    if (regra.termos.some((termo) => termoCasaCategoria(base, termo))) {
      return regra.nome
    }
  }
  ...
  for (const categoria of CATEGORIAS_CATALOGO) {
    const pontuacao = calcularPontuacaoCategoria(base, categoria)
  ...
  return melhorCategoria || obterCategoriaMarcaPorMapa(produto?.marca) || 'Outros produtos'
}
